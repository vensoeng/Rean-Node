...
const getConfig = () => {
  const token = normalize(process.env.NEW_GITHUB_TOKEN);
  const owner = normalize(process.env.OWNER);
  const repo = normalize(process.env.NEW_REPO).replace(/^https?:\/\/github\.com\//i, "").replace(/\.git$/i, "");

  if (!token || !owner || !repo) {
    throw new Error("Missing GitHub config: NEW_GITHUB_TOKEN, OWNER, REPO are required");
  }
